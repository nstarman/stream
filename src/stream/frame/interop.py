...
# LOCAL
from stream.stream.base import SupportsFrame
from stream.stream.core import StreamArm
from stream.stream.plural import StreamArms, StreamArmsBase
from stream.stream.stream import Stream, StreamArmsDescriptor
...
def _fit_stream_StreamArmsDescriptor(
    ...
    **kwargs: Any,
    ...
    out: StreamArms = fit_stream(StreamArms(dict(arms_descr.items())), rot0=rot0, force=force, **kwargs)
    return out
...
def _fit_stream_Stream(
    ...
    **kwargs: Any,
    ...
    newstream._cache["frame_fit_result"] = result
    newstream.flags.set(**asdict(stream.flags))
